...
from resnet_offset_fixed_preprocessing_TFRecords import preprocess   as preprocess_tfrecords

class ResNet_Offset_Fixed():
    ...
    def inference(self, inputs, is_training, input_dims, output_dims, seq_length, scope, dropout_rate = 0.5, return_layer=['logits'], weight_decay=0.0):
        ...
        ############################################################################
        #                Creating ResNet_Offset_Fixed+ LSTM Network Layers                   #
        ############################################################################

        if self.verbose:
            print('Generating ResNet_Offset_Fixed_Preprocessing network layers')
