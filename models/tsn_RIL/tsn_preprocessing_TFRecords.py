...
def preprocess(input_data_tensor, frames, height, width, channel, size, label, istraining, num_seg, input_dims, input_alpha=1.0):
    ...
    for i in range(num_seg):
        ...
        input_seg_tensor = tf.expand_dims(input_seg_tensor, 0)
        input_seg_list.append(input_seg_tensor)

    input_data_tensor = tf.concat(input_seg_list, 0)

    input_data_tensor = tf.reshape(input_data_tensor, [input_dims, tf.to_int32(h_size), tf.to_int32(w_size), channel])
