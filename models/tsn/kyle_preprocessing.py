...
def _get_box_coords(frames, new_height, new_width, h_size, w_size, istraining, scales = [1.]):
    offset_coords = []
    offset_coords.append([tf.div(new_height-1, 2.), tf.div(new_width-1, 2.)])
    offset_coords.append([tf.div(h_size-1, 2.), tf.div(w_size-1, 2.)])
    offset_coords.append([tf.div(h_size-1, 2.), new_width-1 - tf.div(w_size-1, 2.)])
    offset_coords.append([new_height-1 - tf.div(h_size-1, 2.), tf.div(w_size-1, 2.)])
    offset_coords.append([new_height-1 - tf.div(h_size-1, 2.), new_width-1 - tf.div(w_size-1, 2.)])

    if istraining:
        offset_coords = [offset_coords[np.random.randint(len(offset_coords))]]
    ...
    for scale in scales:
        ...
        for offset in offset_coords:
            y1 = tf.div(tf.maximum(0., offset[0]-nh), new_height-1)
            y2 = tf.div(tf.minimum(new_height-1, offset[0]+nh), new_height-1)
            x1 = tf.div(tf.maximum(0., offset[1]-nw), new_width-1)
            x2 = tf.div(tf.minimum(new_width-1, offset[1]+nw), new_width-1)
            boxes.append([y1, x1, y2, x2])

    num_boxes = len(boxes)
    boxes = tf.tile(boxes, [frames, 1])

    box_ind = tf.range(frames)
    box_ind_list = []
    for i in range(num_boxes):
        box_ind_list.append(box_ind)

    box_ind = tf.stack(box_ind_list, axis=1)
    box_ind = tf.reshape(box_ind, [num_boxes*frames])

    return boxes, box_ind


def preprocess(input_data_tensor, frames, height, width, channel, size, label, istraining, num_seg, input_dims, input_alpha):
    ...
    if not istraining:
        num_seg=25
        tick = tf.divide(tf.to_float(frames), float(num_seg))
        tick_h = tf.divide(tick, 2)

        uniform_sample_list = [[tf.cast(tf.minimum(tf.round(tick_h + tick * x), tf.to_float(frames)-1), tf.int32)] for x in range(num_seg)]
        #uniform_sample_list = [[tf.cast(frames-1, tf.int32)]] # This is for last frame
        input_data_tensor = tf.gather_nd(input_data_tensor, uniform_sample_list)

        input_data_tensor = tf.cast(input_data_tensor, tf.float32)
        input_data_tensor = tf.image.resize_images(input_data_tensor, [tf.to_int32(new_height), tf.to_int32(new_width)], align_corners=True)

        boxes, box_ind = _get_box_coords(num_seg, new_height, new_width, h_size, w_size, istraining)

        input_data_tensor = tf.image.crop_and_resize(input_data_tensor, boxes, box_ind, size)
        flipped_data_tensor = tf.map_fn(lambda img: tf.image.flip_left_right(img), input_data_tensor)
        input_data_tensor = tf.concat([input_data_tensor, flipped_data_tensor], 0)
        ...
        SAMPLE_NUM = input_dims/num_seg
        avg_duration = tf.floordiv(frames, num_seg)

        random_indices = []
        for i in range(num_seg):
            random_indices.append(tf.random_uniform([SAMPLE_NUM], avg_duration*i, avg_duration*(i+1), dtype=tf.int32))

        random_indices = tf.stack(random_indices, 1)
        random_indices = tf.reshape(random_indices, [input_dims])
        input_data_tensor = tf.gather(input_data_tensor, random_indices)

        boxes, box_ind = _get_box_coords(SAMPLE_NUM*num_seg, new_height, new_width, h_size, w_size, istraining, [[1., .875, .75, .66][np.random.randint(4)]])

        input_data_tensor = tf.image.crop_and_resize(input_data_tensor, boxes, box_ind, size)
        input_data_tensor = tf.map_fn(lambda img: tf.image.random_flip_left_right(img), input_data_tensor)

        input_data_tensor = _mean_image_subtraction(input_data_tensor, [_R_MEAN, _G_MEAN, _B_MEAN])
        input_data_tensor = tf.map_fn(lambda img: tf.image.rot90(img, 1), input_data_tensor)

        #avg_duration = tf.floordiv(tf.shape(input_data_tensor)[0], num_seg)
        #input_seg_list = []
        #for i in range(num_seg):
        #    input_seg_list.append(tf.random_shuffle(input_data_tensor[i*avg_duration:i*avg_duration+avg_duration, ...]))

        #input_data_tensor = tf.stack(input_seg_list, 1)
        #input_data_tensor = tf.reshape(input_data_tensor, [avg_duration*num_seg, tf.to_int32(h_size), tf.to_int32(w_size), channel])

        #repeat = tf.floordiv(input_dims, tf.shape(input_data_tensor)[0]) + 1
        #input_data_tensor = tf.tile(input_data_tensor, [repeat, 1, 1, 1])

        #input_data_tensor = input_data_tensor[0:input_dims, ...]
        #input_data_tensor = tf.slice(input_data_tensor, [0,0,0,0], tf.stack([input_dims, height, width, channel]))
        #input_data_tensor = tf.reshape(input_data_tensor, tf.stack([input_dims, height, width, channel]))

    return input_data_tensor


def preprocess_new(input_data_tensor, frames, height, width, channel, size, label, istraining, num_seg, input_dims, input_alpha):
    new_height, new_width = _get_new_length(height, width)
    ...
    # For the baseline TSN, reduce again to have frames of input_dims
    input_data_tensor = resample_input(input_data_tensor, footprint, footprint, input_alpha)

    # Uniformly sample down to input_dims
    input_data_tensor = resample_input(input_data_tensor, input_dims, footprint, 1.0)

    #indices = tf.range(0, footprint, tf.div(footprint,input_dims))
    #input_data_tensor = tf.gather(input_data_tensor, indices[:input_dims])
    SAMPLE_NUM = input_dims/num_seg
    ...
    input_data_tensor = tf.stack(input_seg_list, 1)
    input_data_tensor = tf.reshape(input_data_tensor, [SAMPLE_NUM*num_seg, tf.to_int32(h_size), tf.to_int32(w_size), 3])
    input_data_tensor = _mean_image_subtraction(input_data_tensor, [_R_MEAN, _G_MEAN, _B_MEAN])
    input_data_tensor = tf.map_fn(lambda img: tf.image.rot90(img, 1), input_data_tensor)
