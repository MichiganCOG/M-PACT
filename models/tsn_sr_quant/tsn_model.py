...
class TSN_SR_QUANT():
    def __init__(self, input_dims, output_dims, exp_name, num_vids, num_seg=3, init=False, model_alpha=1.0, input_alpha=1.0, verbose=True):
        self.name        = 'tsn_sr_quant'
	...
	self.store_alpha = True
        ...
        if self.verbose:
            print('TSN_SR_QUANT initialized')
    ...
    def preprocess_tfrecords(self, input_data_tensor, frames, height, width, channel, input_dims, output_dims, seq_length, size, label, istraining, video_step):
        output, alpha_tensor = preprocess_tfrecords(input_data_tensor, frames, height, width, channel, size, label, istraining, self.num_seg, self.input_dims, self.model_alpha, self.input_alpha, self.num_vids, video_step)
