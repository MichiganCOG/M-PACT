...
def preprocess_image(image, output_height, output_width, is_training=False,
    ...
    Args:
        ...
        :is_training:    `True` if we're preprocessing the image for training and
                         `False` otherwise.
        :resize_side_min: The lower bound for the smallest side of the image for
                          aspect-preserving resizing. If `is_training` is `False`, 
                          then this value is used for rescaling.
        :resize_side_max: The upper bound for the smallest side of the image for
                          aspect-preserving resizing. If `is_training` is `False`, 
                          this value is ignored. Otherwise, the resize side is 
                          sampled from  [resize_size_min, resize_size_max].
    Returns:
        A preprocessed image.
    """
    if is_training:
...
def preprocess(index, data, labels, size, is_training):
    ...
    # Double the size of input since LSTM need maximum possible sequence size and that is seq_length (2*orig_input_dims)
    if is_training:
