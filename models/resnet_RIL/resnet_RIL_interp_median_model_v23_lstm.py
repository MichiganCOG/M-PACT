" RESNET-50 + RAIN (INTERP + MEDIAN + LSTM) v23 + LSTM MODEL IMPLEMENTATION FOR USE WITH TENSORFLOW "
...
class ResNet_RIL_Interp_Median_v23_lstm():
    ...
    def __init__(self, verbose=True):
        ...
        self.name = 'resnet_RIL_interp_median_model_v23_lstm'
        print "resnet RIL interp median v23_lstm initialized"
    ...
    def _extraction_layer(self, inputs, params, sets, K, L):
        ...
        sample_alpha_tick = tf.exp(-tf.nn.relu(params[0][0]))
        ...
        d1 = (output_idx - x0)
        d2 = (x1 - x0)
        d1 = tf.reshape(tf.tile(d1, [54*54*1]), [L,54,54,1])
        d2 = tf.reshape(tf.tile(d2, [54*54*1]), [L,54,54,1])
    ...
    def inference(self, inputs, is_training, input_dims, output_dims, seq_length, scope, k, j, dropout_rate = 0.5, return_layer='logits', data_dict=None, weight_decay=0.0):
        ...
        if self.verbose:
            print('Generating RESNET RAIN INTERP MEDIAN v23_lstm network layers')
        ...
        with tf.name_scope(scope, 'resnet', [inputs]):
            ...
            #############################################################################
            layers['RAINlayer_lstm']    = self._LSTM2(tf.zeros((50, 54, 54, 256 )), seq_length, nfilters=256, feat_size=2916, cell_size=128, reuse=False)

            layers['RAINlayer_lstm_fc'] = self.FClayer(inputs=layers['RAINlayer_lstm'],
                                                     op_dims=1, reuse=False)

            layers['RAINlayer']         = self._extraction_layer(inputs=layers['26'], params=layers['RAINlayer_lstm_fc'], sets=j, K=k, L=seq_length)

            # Step 2
            layers['RAINlayer_lstm_2']    = self._LSTM2(layers['RAINlayer'], seq_length, nfilters=256, feat_size=2916, cell_size=128, reuse=True)

            layers['RAINlayer_lstm_fc_2'] = self.FClayer(inputs=layers['RAINlayer_lstm_2'],
                                                     op_dims=1, reuse=True)
            layers['RAINlayer_2']         = self._extraction_layer(inputs=layers['26'], params=layers['RAINlayer_lstm_fc_2'], sets=j, K=k, L=seq_length)

            # Step 3
            layers['RAINlayer_lstm_3']    = self._LSTM2(layers['RAINlayer_2'], seq_length, nfilters=256, feat_size=2916, cell_size=128, reuse=True)

            layers['RAINlayer_lstm_fc_3'] = self.FClayer(inputs=layers['RAINlayer_lstm_3'],
                                                     op_dims=1, reuse=True)
            layers['RAINlayer_3']         = self._extraction_layer(inputs=layers['26'], params=layers['RAINlayer_lstm_fc_3'], sets=j, K=k, L=seq_length)

            # Step 4
            layers['RAINlayer_lstm_4']    = self._LSTM2(layers['RAINlayer_3'], seq_length, nfilters=256, feat_size=2916, cell_size=128, reuse=True)

            layers['RAINlayer_lstm_fc_4'] = self.FClayer(inputs=layers['RAINlayer_lstm_4'],
                                                     op_dims=1, reuse=True)
            layers['RAINlayer_4']         = self._extraction_layer(inputs=layers['26'], params=layers['RAINlayer_lstm_fc_4'], sets=j, K=k, L=seq_length)

            ############################################################################

            #########
            layers.update(self._conv_block([128,128,512], kernel_size=3, name='3a', layer_numbers=['27','28','29','30','31','32','33','34','35'],
                            input_layer=layers['RAINlayer_4'], data_dict=data_dict))
