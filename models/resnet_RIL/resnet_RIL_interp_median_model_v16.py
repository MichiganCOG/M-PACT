" RESNET-50 + RAIN (INTERP + MEDIAN) v16 + LSTM MODEL IMPLEMENTATION FOR USE WITH TENSORFLOW "
...
class ResNet_RIL_Interp_Median_v16():
    ...
    def __init__(self, verbose=True):
        ...
        self.name = 'resnet_RIL_interp_median_model_v16'
        print "resnet RIL interp median v16 initialized"
    ...
    def inference(self, inputs, is_training, input_dims, output_dims, seq_length, scope, k, j, dropout_rate = 0.5, return_layer='logits', data_dict=None, weight_decay=0.0):
        ...
        if self.verbose:
            print('Generating RESNET RAIN INTERP MEDIAN v16 network layers')
        ...
        #data_dict = h5py.File('resnet50_weights_tf_dim_ordering_tf_kernels.h5','r')
        ...
        with tf.name_scope(scope, 'resnet', [inputs]):
            ...
            layers['Conv1'] = conv_layer(input_tensor=inputs,
                    filter_dims=[16, 16, 128], stride_dims=[8,8],
                    padding = 'VALID',
                    name='Conv1',
                    weight_decay = weight_decay, non_linear_fn=None)
            ...
            # Utilize a higher set of filters to extract more visually important features
            layers['Conv2'] = conv_layer(input_tensor=layers['Conv1_bn'],
                    filter_dims=[1, 1, 32], stride_dims=[1,1],
                    padding = 'VALID',
                    name='Conv2',
                    kernel_init=tf.constant_initializer(data_dict['res3b_branch2a']['res3b_branch2a_W:0'].value[:,:,:32,:]),
                    bias_init=tf.constant_initializer(data_dict['res3b_branch2a']['res3b_branch2a_b:0'].value[:32]),
                    weight_decay = weight_decay, non_linear_fn=tf.nn.relu)

            layers['Conv2_bn'] = tf.layers.batch_normalization(layers['Conv2'],
                    name='bn_Conv2',
                    moving_mean_initializer=tf.constant_initializer(data_dict['bn3b_branch2a']['bn3b_branch2a_running_mean:0'].value[:32]),
                    moving_variance_initializer=tf.constant_initializer(data_dict['bn3b_branch2a']['bn3b_branch2a_running_std:0'].value[:32]),
                    beta_initializer=tf.constant_initializer(data_dict['bn3b_branch2a']['bn3b_branch2a_beta:0'].value[:32]),
                    gamma_initializer=tf.constant_initializer(data_dict['bn3b_branch2a']['bn3b_branch2a_gamma:0'].value[:32])
)
            layers['POOL'] = max_pool_layer(tf.nn.relu(layers['Conv2_bn']),
                                         filter_dims=[3, 3], stride_dims=[3,3],
                                         name='pool1', padding='VALID')

            layers['Reshape1'] = tf.reshape(layers['POOL'], (-1, k, 9, 9, 32))

            layers['Dimshuffle1'] = tf.transpose(layers['Reshape1'], (0,2,3,4,1))

            layers['Reshape2'] = tf.reshape(layers['Dimshuffle1'], (-1, k))

            layers['FC1'] = fully_connected_layer(input_tensor=layers['Reshape2'],
                    out_dim=16, non_linear_fn=None,
                    name='FC1', weight_decay=weight_decay)

            layers['Reshape3'] = tf.reshape(layers['FC1'], (-1, 9, 9, 32, 16))

            layers['Reshape4'] = tf.reshape(layers['Reshape3'], (-1, 32*16))

            layers['FC2'] = fully_connected_layer(input_tensor=layers['Reshape4'],
                    out_dim=32, non_linear_fn=None,
                    name='FC2', weight_decay=weight_decay)

            layers['Reshape5'] = tf.reshape(layers['FC2'], (-1, 9*9*32))

            layers['FC3'] = fully_connected_layer(input_tensor=layers['Reshape5'],
                    out_dim=2, non_linear_fn=tf.nn.sigmoid,
                    name='FC3', weight_decay=weight_decay)
            ...
            layers['1'] = conv_layer(input_tensor=layers['RIlayer'],
                    filter_dims=[7, 7, 64], stride_dims=[1,1],
                    padding = 'VALID',
                    name='conv1',
...
if __name__ == '__main__':
    XX = ResNet_RIL_Interp_Median_v16()
    llol = XX.inference(tf.placeholder(tf.float32, shape=[250,224,224,3]), True, 250, 51, 50, 'mine', 25, 10, dropout_rate = 0.5, return_layer='logits', data_dict=None, weight_decay=0.0)
