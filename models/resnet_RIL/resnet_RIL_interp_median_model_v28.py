" RESNET-50 + RAIN (INTERP + MEDIAN) v28 + LSTM MODEL IMPLEMENTATION FOR USE WITH TENSORFLOW "
...
class ResNet_RIL_Interp_Median_v28():
    ...
    def __init__(self, verbose=True):
        ...
        self.name = 'resnet_RIL_interp_median_model_v28'
        print "resnet RAIN interp median v28 initialized"
    ...
    def _extraction_layer_1(self, inputs, params, sets, K, L):
        ...
        d1 = (output_idx - x0)
        d2 = (x1 - x0)
        d1 = tf.reshape(tf.tile(d1, [shp_feat]), [tf.cast((sets*K) - sample_phi_tick, tf.int32),shp_feat])
        d2 = tf.reshape(tf.tile(d2, [shp_feat]), [tf.cast((sets*K) - sample_phi_tick, tf.int32),shp_feat])
	...
	# Create y0 and y1 outputs
        ...
        output = tf.reshape(output, (tf.cast((sets*K) - sample_phi_tick, tf.int32), shp_feat), name='RIlayeroutput_p1')
	...
	# Create y0 and y1 outputs
        ...
        output = tf.reshape(output, (L, shp_feat), name='RIlayeroutput_final')
    ...
    def inference(self, inputs, is_training, input_dims, output_dims, seq_length, scope, k, j, dropout_rate = 0.5, return_layer='logits', data_dict=None, weight_decay=0.0):
        ...
        if self.verbose:
            print('Generating RESNET RAIN INTERP MEDIAN v28 network layers')
	    ...
	    #layers['RELU'] = tf.nn.relu(layers['Conv1_bn'])
            ...
            layers['RAINlayer'] = self._extraction_layer_1(inputs=layers['124'],
