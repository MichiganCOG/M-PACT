" RESNET-50 + RAIN (INTERP + MEDIAN + LSTM) v37_1_LSTM + LSTM MODEL IMPLEMENTATION FOR USE WITH TENSORFLOW "
...
class ResNet_RIL_Interp_Median_v37_1_lstm():
    ...
    def __init__(self, verbose=True):
        ...
        self.name = 'resnet_RIL_interp_median_model_v37_1_lstm'
        print "resnet RAIN interp median v37_1_lstm initialized"
    ...
    def _extraction_layer2(self, inputs, params, sets, K, L):
        """
        Args:
            :inputs: Original inputs to the model
            :params: Offset and sampling parameter estimates from Parameterization Network
    ...
    def inference(self, inputs, is_training, input_dims, output_dims, seq_length, scope, k, j, dropout_rate = 0.5, return_layer=['logits'], data_dict=None, weight_decay=0.0):
        ...
        if self.verbose:
            print('Generating RESNET RAIN INTERP MEDIAN v37_1_lstm network layers')
        ...
        with tf.name_scope(scope, 'resnet', [inputs]):
            ...
            layers['RAINlayer_step4']         = self._extraction_layer2(inputs=layers['124'], params=layers['Parameterization_Variables_phi'], sets=j, K=k, L=seq_length)
