" RESNET-50 + RAIN (INTERP + MEDIAN + LSTM) v26 + LSTM MODEL IMPLEMENTATION FOR USE WITH TENSORFLOW "
...
class ResNet_RIL_Interp_Median_v26_lstm():
    ...
    def __init__(self, verbose=True):
        ...
        self.name = 'resnet_RIL_interp_median_model_v26_lstm'
        print "resnet RAIN interp median v26_lstm initialized"
    ...
    def _extraction_layer(self, inputs, params, sets, K, L):
        ...
        sample_phi_tick   = tf.exp(-tf.nn.relu(params[0][0]))
    ...
    def inference(self, inputs, is_training, input_dims, output_dims, seq_length, scope, k, j, dropout_rate = 0.5, return_layer=['logits'], data_dict=None, weight_decay=0.0):
        ...
        if self.verbose:
            print('Generating RESNET RAIN INTERP MEDIAN v26_lstm network layers')
	    ...
	    #layers['RELU'] = tf.nn.relu(layers['Conv1_bn'])
            ...
            layers['RAINlayer_step4']         = self._extraction_layer(inputs=layers['124'], params=layers['Parameterization_Variables'], sets=j, K=k, L=seq_length)
