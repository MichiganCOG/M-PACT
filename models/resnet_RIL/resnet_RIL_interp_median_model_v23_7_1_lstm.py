" RESNET-50 + RAIN (INTERP + MEDIAN + LSTM) v23_7_1 + LSTM MODEL IMPLEMENTATION FOR USE WITH TENSORFLOW "
...
class ResNet_RIL_Interp_Median_v23_7_1_lstm():
    ...
    def __init__(self, verbose=True):
        ...
        self.name = 'resnet_RIL_interp_median_model_v23_7_1_lstm'
        print "resnet RIL interp median v23_7_1_lstm initialized"
    ...
    def _extraction_layer(self, inputs, params, sets, K, L):
        ...
        sample_alpha_tick = tf.cond(params[0][0] > 0, lambda: tf.tanh(tf.nn.relu(params[0][0]))*0.5 + 0.5, lambda: tf.tanh(tf.nn.relu(-params[0][0]))*0.5 + 0.5)
    ...
    def inference(self, inputs, is_training, input_dims, output_dims, seq_length, scope, k, j, dropout_rate = 0.5, return_layer=['logits'], data_dict=None, weight_decay=0.0):
        ...
        if self.verbose:
            print('Generating RESNET RAIN INTERP MEDIAN v23_7_1_lstm network layers')
        ...
        with tf.name_scope(scope, 'resnet', [inputs]):
            ...
            layers['RAINlayer_step1']         = self._extraction_layer(inputs=layers['26'], params=layers['RAINlayer_lstm_fc_step1'], sets=j, K=k, L=seq_length)
            ...
            layers['RAINlayer_step2']         = self._extraction_layer(inputs=layers['26'], params=layers['RAINlayer_lstm_fc_step2'], sets=j, K=k, L=seq_length)
            ...
            layers['RAINlayer_step3']         = self._extraction_layer(inputs=layers['26'], params=layers['RAINlayer_lstm_fc_step3'], sets=j, K=k, L=seq_length)
            ...
            layers['RAINlayer_step4']         = self._extraction_layer(inputs=layers['26'], params=layers['Parameterization_Variables'], sets=j, K=k, L=seq_length)
