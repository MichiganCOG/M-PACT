" RESNET-50 + RAIN (INTERP + MAX) v20 + LSTM MODEL IMPLEMENTATION FOR USE WITH TENSORFLOW "
...
class ResNet_RIL_Interp_Max_v20():
    ...
    def __init__(self, verbose=True):
        ...
        self.name = 'resnet_RIL_interp_max_model_v20'
        print "resnet RAIN interp max v20 initialized"
    ...
    def _extraction_layer(self, inputs, params, sets, K, L):
        ...
        sample_phi_tick   = tf.nn.top_k(params[:,0], 1).values[0]

        # Extract shape of input signal
        frames, feat_dim = inputs.get_shape().as_list()

        # Offset scaling to match inputs temporal dimension
        sample_phi_tick = sample_phi_tick * tf.cast(K, tf.float32)
        ...
        d1 = (output_idx - x0)
        d2 = (x1 - x0)
        d1 = tf.reshape(tf.tile(d1, [2048]), [L,2048])
        d2 = tf.reshape(tf.tile(d2, [2048]), [L,2048])
	...
	# Create y0 and y1 outputs
        ...
        output = tf.reshape(output, (L, feat_dim), name='RIlayeroutput')
    ...
    def inference(self, inputs, is_training, input_dims, output_dims, seq_length, scope, k, j, dropout_rate = 0.5, return_layer='logits', data_dict=None, weight_decay=0.0):
        ...
        if self.verbose:
            print('Generating RESNET RAIN INTERP MAX v20 network layers')
	    ...
	    #layers['RELU'] = tf.nn.relu(layers['Conv1_bn'])
            ...
            layers['FC1'] = fully_connected_layer(input_tensor=layers['124'],
                    out_dim=256, non_linear_fn=tf.nn.crelu,
                    name='FC1', weight_decay=weight_decay)

            layers['Reshape2'] = tf.reshape(layers['FC1'], (-1, k*256))

            layers['FC2'] = fully_connected_layer(input_tensor=layers['Reshape2'],
                    out_dim=512, non_linear_fn=tf.nn.crelu,
                    name='FC2', weight_decay=weight_decay)

            layers['FC3'] = fully_connected_layer(input_tensor=layers['FC2'],
                    out_dim=1, non_linear_fn=tf.nn.softmax,
