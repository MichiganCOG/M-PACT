" RESNET-50 + RAIN (INTERP + MEDIAN) v23_2 + LSTM MODEL IMPLEMENTATION FOR USE WITH TENSORFLOW "
...
class ResNet_RIL_Interp_Median_v23_2():
    ...
    def __init__(self, verbose=True):
        ...
        self.name = 'resnet_RIL_interp_median_model_v23_2'
        print "resnet RIL interp median v23_2 initialized"
    ...
    def inference(self, inputs, is_training, input_dims, output_dims, seq_length, scope, k, j, dropout_rate = 0.5, return_layer='logits', data_dict=None, weight_decay=0.0):
        ...
        if self.verbose:
            print('Generating RESNET RAIN INTERP MEDIAN v23_2 network layers')
        ...
        with tf.name_scope(scope, 'resnet', [inputs]):
            ...
            layers['Parameterization_Variables'] = [tf.nn.sigmoid(tf.get_variable('alpha',shape=[], dtype=tf.float32, initializer=tf.constant_initializer(0.69)))]
