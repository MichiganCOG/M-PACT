" RESNET-50 + RAIN (INTERP + MEDIAN) v27 + LSTM MODEL IMPLEMENTATION FOR USE WITH TENSORFLOW "
...
class ResNet_RIL_Interp_Median_v27():
    ...
    def __init__(self, verbose=True):
        ...
        self.name = 'resnet_RIL_interp_median_model_v27'
        print "resnet RIL interp median v27 initialized"

    def _extraction_layer_1(self, inputs, params, sets, K, L):
        """
        Args:
	...
	# Create y0 and y1 outputs
        ...
        output = tf.reshape(output, (tf.cast((sets*K) - sample_phi_tick, tf.int32), shp_h, shp_w, channel), name='RIlayeroutput_p1')

        output = self._extraction_layer_2(tf.cast((sets*K) - sample_phi_tick, tf.int32), output, params, L)

        return output


    def _extraction_layer_2(self, frames, inputs, params, L):
        """
        Args:
            :inputs: Output of extraction layer 1
            :params: Offset and sampling parameter estimates from Parameterization Network
            :L:      Expected number of output frames
        ...
        # Extract shape of input signal
        _, shp_h, shp_w, channel = inputs.get_shape().as_list()
	...
	# Create y0 and y1 outputs
        ...
        output = tf.reshape(output, (L, shp_h, shp_w, channel), name='RIlayeroutput_final')
    ...
    def inference(self, inputs, is_training, input_dims, output_dims, seq_length, scope, k, j, dropout_rate = 0.5, return_layer='logits', data_dict=None, weight_decay=0.0):
        ...
        if self.verbose:
            print('Generating RESNET RAIN INTERP MEDIAN v27 network layers')
        ...
        with tf.name_scope(scope, 'resnet', [inputs]):
            ...
            layers['RIlayer'] = self._extraction_layer_1(inputs=inputs,
