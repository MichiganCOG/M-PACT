" RESNET-50 + RAIN (INTERP + MEDIAN) v10 + LSTM MODEL IMPLEMENTATION FOR USE WITH TENSORFLOW "
...
class ResNet_RIL_Interp_Median_v10():
    ...
    def __init__(self, verbose=True):
        ...
        self.name = 'resnet_RIL_interp_median_model_v10'
        print "resnet RIL interp median v10 initialized"
    ...
    def _extraction_layer(self, inputs, params, sets, K, L):
        ...
        sample_phi_tick = sample_phi_tick * tf.cast(L, tf.float32)

        phi_tick = tf.tile([sample_phi_tick], [L])

        phi_tick = phi_tick + tf.slice(tf.range(start=1., limit=float(L)+1., delta=1., dtype=tf.float32), [0], [L])
    ...
    def inference(self, inputs, is_training, input_dims, output_dims, seq_length, scope, k, j, dropout_rate = 0.5, return_layer='logits', data_dict=None, weight_decay=0.0):
        ...
        if self.verbose:
            print('Generating RESNET RAIN INTERP MEDIAN v10 network layers')
