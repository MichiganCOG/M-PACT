" RESNET-50 + RAIN (INTERP + MAX + NOSORT) v4 + LSTM MODEL IMPLEMENTATION FOR USE WITH TENSORFLOW "
...
class ResNet_RIL_Interp_Max_Nosort_v4():
    ...
    def __init__(self, verbose=True):
        ...
        self.name = 'resnet_RIL_interp_max_nosort_v4'
        print "resnet RIL interp max nosort v4 initialized"

    def _extraction_layer(self, inputs, params, sets, K, L):
        """
        Args:
            :inputs: Original inputs to the model
            :params: Index estimates from Parameterization Network
        ...
        # Index estimates
        output_idx = tf.cast(tf.nn.top_k(tf.transpose(params),1).values[:], tf.float32)
        ...
        d1 = (output_idx - x0)
        d2 = (x1 - x0)
        d1 = tf.reshape(tf.tile(d1[:,0], [224*224*3]), [L,224,224,3])
        d2 = tf.reshape(tf.tile(d2[:,0], [224*224*3]), [L,224,224,3])
	...
	# Create y0 and y1 outputs
        output_0 = tf.gather(inputs, output_idx_0[:,0]-1)
        output_1 = tf.gather(inputs, output_idx_1[:,0]-1)
        output   = tf.gather(inputs, output_idx[:,0]-1)
    ...
    def inference(self, inputs, is_training, input_dims, output_dims, seq_length, scope, k, j, dropout_rate = 0.5, return_layer='logits', data_dict=None, weight_decay=0.0):
        ...
        if self.verbose:
            print('Generating RESNET RAIN INTERP MEAN NOSORT v4 network layers')
        ...
        with tf.name_scope(scope, 'resnet', [inputs]):
            ...
            layers['FC2'] = fully_connected_layer(input_tensor=layers['Reshape4'],
                    out_dim=seq_length, non_linear_fn=tf.nn.sigmoid,
