" RESNET-50 + RAIN (INTERP + MEDIAN) v23_8_1 + LSTM MODEL IMPLEMENTATION FOR USE WITH TENSORFLOW "
...
class ResNet_RIL_Interp_Median_v23_8_1():
    ...
    def __init__(self, verbose=True):
        ...
        self.name = 'resnet_RIL_interp_median_model_v23_8_1'
        print "resnet RIL interp median v23_8_1 initialized"
    ...
    def _extraction_layer(self, inputs, params, sets, K, L):
        ...
        sample_alpha_tick = tf.cond(params[0] > 0, lambda: tf.nn.sigmoid(tf.nn.relu(params[0])), lambda: tf.nn.sigmoid(tf.nn.relu(-params[0])))
    ...
    def inference(self, inputs, is_training, input_dims, output_dims, seq_length, scope, k, j, dropout_rate = 0.5, return_layer=['logits'], data_dict=None, weight_decay=0.0):
        ...
        if self.verbose:
            print('Generating RESNET RAIN INTERP MEDIAN v23_8_1 network layers')
        ...
        with tf.name_scope(scope, 'resnet', [inputs]):
            ...
            layers['Parameterization_Variables'] = [tf.get_variable('alpha',shape=[], dtype=tf.float32, initializer=tf.constant_initializer(1.0))]
