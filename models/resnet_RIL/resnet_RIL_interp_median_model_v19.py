" RESNET-50 + RAIN (INTERP + MEDIAN) v19 + LSTM MODEL IMPLEMENTATION FOR USE WITH TENSORFLOW "
...
class ResNet_RIL_Interp_Median_v19():
    ...
    def __init__(self, verbose=True):
        ...
        self.name = 'resnet_RIL_interp_median_model_v19'
        print "resnet RAIN interp median v19 initialized"
    ...
    def _extraction_layer(self, inputs, params, sets, K, L):
        ...
        sample_alpha_tick = tf.exp(-tf.nn.top_k(params[:,0], params[:,0].get_shape()[0]//2).values[params[:,0].get_shape()[0]//2 - 1])
        sample_phi_tick   = tf.exp(-tf.nn.top_k(params[:,1], params[:,1].get_shape()[0]//2).values[params[:,1].get_shape()[0]//2 - 1])
    ...
    def inference(self, inputs, is_training, input_dims, output_dims, seq_length, scope, k, j, dropout_rate = 0.5, return_layer='logits', data_dict=None, weight_decay=0.0):
        ...
        if self.verbose:
            print('Generating RESNET RAIN INTERP MEDIAN v19 network layers')
