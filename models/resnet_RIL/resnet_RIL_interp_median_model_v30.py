" RESNET-50 + RAIN (INTERP + MEDIAN) v30 + LSTM MODEL IMPLEMENTATION FOR USE WITH TENSORFLOW "
...
class ResNet_RIL_Interp_Median_v30():
    ...
    def __init__(self, verbose=True):
        ...
        self.name = 'resnet_RIL_interp_median_model_v30'
        print "resnet RAIN interp median v30 initialized"




    def _extraction_layer_phi(self, inputs, params, sets, K, L):
        """
        Args:
            :inputs: Inputs to network
            :params: Offset parameter estimates from Parameterization Network
        ...
        Return:
            :output: Phi applied to the inputs
        """
        ...
        phi_tick = tf.tile([sample_phi_tick], [tf.cast((sets*K) - sample_phi_tick, tf.int32)])

        # Generate indices for output
        output_idx = tf.range(start=1, limit=tf.cast((sets*K), tf.float32), delta=1., dtype=tf.float32)

        output_idx = tf.slice(output_idx, [0],[tf.cast((sets*K) - sample_phi_tick, tf.int32)])
        ...
        d1 = (output_idx - x0)
        d2 = (x1 - x0)
        d1 = tf.reshape(tf.tile(d1, [224*224*3]), [tf.cast((sets*K) - sample_phi_tick, tf.int32),224,224,3])
        d2 = tf.reshape(tf.tile(d2, [224*224*3]), [tf.cast((sets*K) - sample_phi_tick, tf.int32),224,224,3])
	...
	# Create y0 and y1 outputs
        ...
        output = tf.reshape(output, (tf.cast((sets*K) - sample_phi_tick, tf.int32), shp_h, shp_w, channel), name='RIlayeroutput_phi')

        L_intermediate = (sets*K) - sample_phi_tick

        return output, L_intermediate




    def _extraction_layer_alpha(self, inputs, params, sets, K, L, L_intermediate):
        """
        Args:
            :inputs: Previous layer of model
            :params: Sampling parameter estimates from Parameterization Network
            :sets:   Number of non overlapping sets obtained from applying a temporal slid
                     ing window over the input
            :K:      Size of temporal sliding window
            :L:      Expected number of output frames
        ...
        # Extract shape of input signal
        _, shp_feat = inputs.get_shape().as_list()
        frames = L_intermediate
        ...
        alpha_tick = sample_alpha_tick * tf.cast(frames, tf.float32) / (float(L))

        # Include sampling parameter to correct output indices
        output_idx = tf.multiply(tf.tile([alpha_tick], [L]), output_idx)

        # Clip output index values to >= 1 and <=N (valid cases only)
        output_idx = tf.clip_by_value(output_idx, 1., tf.cast(frames, tf.float32))

        # Create x0 and x1 float
        x0 = tf.clip_by_value(tf.floor(output_idx), 1., tf.cast(frames, tf.float32)-1.)
        x1 = tf.clip_by_value(tf.floor(output_idx+1.), 2., tf.cast(frames, tf.float32))
	...
	# Create y0 and y1 outputs
        ...
        output = tf.reshape(output, (L, shp_feat), name='RIlayeroutput_alpha')
    ...
    def inference(self, inputs, is_training, input_dims, output_dims, seq_length, scope, k, j, dropout_rate = 0.5, return_layer='logits', data_dict=None, weight_decay=0.0):
        ...
        if self.verbose:
            print('Generating RESNET RAIN INTERP MEDIAN v30 network layers')
        ...
        with tf.name_scope(scope, 'resnet', [inputs]):
            ...
            layers['Parameterization_Variable_Phi'] = [tf.get_variable('phi',shape=[], dtype=tf.float32, initializer=tf.constant_initializer(0.69))]

            layers['RAINlayer1'], L_intermediate = self._extraction_layer_phi(inputs=inputs,
                                                       params=layers['Parameterization_Variable_Phi'],
	    ...
	    #layers['RELU'] = tf.nn.relu(layers['Conv1_bn'])
            ...
            layers['1'] = conv_layer(input_tensor=layers['RAINlayer1'],
                    filter_dims=[7, 7, 64], stride_dims=[2,2],
                    padding = 'VALID',
                    name='conv1',
                    kernel_init=tf.constant_initializer(data_dict['conv1']['conv1_W:0'].value),
	    ...
	    #layers['RELU'] = tf.nn.relu(layers['Conv1_bn'])
            ...
            layers['Parameterization_Variable_Alpha'] = [tf.get_variable('alpha',shape=[], dtype=tf.float32, initializer=tf.constant_initializer(0.69))]


            layers['RAINlayer2'] = self._extraction_layer_alpha(inputs=layers['124'],
                                                       params=layers['Parameterization_Variable_Alpha'],
                                                       sets=j, L=seq_length, K=k, L_intermediate=L_intermediate)

            ############################################################################

            layers['125'] = self._LSTM(layers['RAINlayer2'], seq_length, feat_size=2048, cell_size=512)
