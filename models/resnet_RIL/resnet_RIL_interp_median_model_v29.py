" RESNET-50 + RAIN (INTERP + MEDIAN) v29 + LSTM MODEL IMPLEMENTATION FOR USE WITH TENSORFLOW "
...
class ResNet_RIL_Interp_Median_v29():
    ...
    def __init__(self, verbose=True):
        ...
        self.name = 'resnet_RIL_interp_median_model_v29'
        print "resnet RAIN interp median v29 initialized"


    def _extraction_layer_1(self, inputs, params, sets, K, L_intermediate):
        """
        Args:
            ...
            :L_intermediate:      Expected number of output frames
        ...
        output_idx = tf.range(start=1., limit=float(L_intermediate)+1., delta=1., dtype=tf.float32)

        output_idx = tf.slice(output_idx, [0],[L_intermediate])

        # Sampling parameter scaling to match inputs temporal dimension
        alpha_tick = sample_alpha_tick * tf.cast(K * sets, tf.float32) / (float(L_intermediate))

        # Include sampling parameter to correct output indices
        output_idx = tf.multiply(tf.tile([alpha_tick], [L_intermediate]), output_idx)
        ...
        d1 = (output_idx - x0)
        d2 = (x1 - x0)
        d1 = tf.reshape(tf.tile(d1, [224*224*3]), [L_intermediate,224,224,3])
        d2 = tf.reshape(tf.tile(d2, [224*224*3]), [L_intermediate,224,224,3])
	...
	# Create y0 and y1 outputs
        ...
        output = tf.reshape(output, (L_intermediate, shp_h, shp_w, channel), name='RIlayeroutput')
    ...
    def _extraction_layer_2(self, inputs, params, sets, K, L):
        ...
        sample_phi_tick = sample_phi_tick * tf.cast((frames) - L, tf.float32)
        ...
        # Sampling parameter scaling to match inputs temporal dimension
        alpha_tick = tf.cast(frames, tf.float32) / (float(L) + sample_phi_tick)
    ...
    def inference(self, inputs, is_training, input_dims, output_dims, seq_length, scope, k, j, dropout_rate = 0.5, return_layer='logits', data_dict=None, weight_decay=0.0):
        ...
        if self.verbose:
            print('Generating RESNET RAIN INTERP MEDIAN v29 network layers')
        ...
        with tf.name_scope(scope, 'resnet', [inputs]):
            ...
            layers['RAINlayer1'] = self._extraction_layer_1(inputs=inputs,
                                                       params=layers['Parameterization_Variable_Alpha'],
                                                       sets=j, L_intermediate=100, K=k)
	    ...
	    #layers['RELU'] = tf.nn.relu(layers['Conv1_bn'])
            ...
            layers['RAINlayer2'] = self._extraction_layer_2(inputs=layers['124'],
