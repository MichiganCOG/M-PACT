" RESNET-50 + RAIN (INTERP + MEDIAN) v40 + LSTM MODEL IMPLEMENTATION FOR USE WITH TENSORFLOW "
...
class ResNet_RIL_Interp_Median_v40():
    ...
    def __init__(self, verbose=True):
        ...
        self.name = 'resnet_RIL_interp_median_model_v40'
        print "resnet RIL interp median v40 initialized"
    ...
    def _extraction_layer(self, inputs, params, sets, K, L):
        ...
        Args:
            ...
            :L_inter Intermediate number of frames passed to extraction layer 2
        ...
        sample_alpha_tick = tf.cond(params[0] > 0, lambda: tf.tanh(tf.nn.relu(params[0]))*0.5 + 0.5, lambda: tf.tanh(tf.nn.relu(-params[0]))*0.5 + 0.5)
	...
	# Create y0 and y1 outputs
        ...
        output = tf.reshape(output, (L, shp_h, shp_w, channel), name='RIlayeroutput_inter')
    ...
    def inference(self, inputs, is_training, input_dims, output_dims, seq_length, scope, k, j, dropout_rate = 0.5, return_layer=['logits'], weight_decay=0.0):
        ...
        if self.verbose:
            print('Generating RESNET RAIN INTERP MEDIAN v40 network layers')
        ...
        with tf.name_scope(scope, 'resnet', [inputs]):
            ...
            layers['Parameterization_Variables'] = [tf.get_variable('alpha',shape=[], dtype=tf.float32, initializer=tf.constant_initializer(0.5))]
            layers['Parameterization_Variables_phi'] = [tf.get_variable('phi',shape=[], dtype=tf.float32, initializer=tf.constant_initializer(0.5))]
            ...
            layers['1'] = conv_layer(input_tensor=layers['RIlayer_2'],
