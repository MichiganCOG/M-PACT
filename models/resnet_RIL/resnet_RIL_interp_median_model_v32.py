" RESNET-50 + RAIN (INTERP + MEDIAN) v32 + LSTM MODEL IMPLEMENTATION FOR USE WITH TENSORFLOW "
...
class ResNet_RIL_Interp_Median_v32():
    ...
    def __init__(self, verbose=True):
        ...
        self.name = 'resnet_RIL_interp_median_model_v32'
        print "resnet RIL interp median v32 initialized"
	...
	# Create y0 and y1 outputs
        ...
        output = tf.reshape(output, (L, shp_feat), name='RIlayeroutput')
    ...
    def inference(self, inputs, is_training, input_dims, output_dims, seq_length, scope, k, j, dropout_rate = 0.5, return_layer='logits', data_dict=None, weight_decay=0.0):
        ...
        if self.verbose:
            print('Generating RESNET RAIN INTERP MEDIAN v32 network layers')
        ...
        return [layers['Parameterization_Variables']]#layers['Parameterization_Variables'][0], layers['logits']]
