" RESNET-50 + RAIN (INTERP + MEDIAN + LSTM) v24 + LSTM MODEL IMPLEMENTATION FOR USE WITH TENSORFLOW "
...
class ResNet_RIL_Interp_Median_v24_lstm():
    ...
    def __init__(self, verbose=True):
        ...
        self.name = 'resnet_RIL_interp_median_model_v24_lstm'
        print "resnet RAIN interp median v24_lstm initialized"
    ...
    def _LSTM2(self, inputs, seq_length, feat_size, cell_size=1024, reuse=False):
        """
        Args:
        ...
        lstm_outputs = tf.stack(states[-1])
    ...
    def inference(self, inputs, is_training, input_dims, output_dims, seq_length, scope, k, j, dropout_rate = 0.5, return_layer='logits', data_dict=None, weight_decay=0.0):
        ...
        if self.verbose:
            print('Generating RESNET RAIN INTERP MEDIAN v24_lstm network layers')
        ...
        with tf.name_scope(scope, 'resnet', [inputs]):
            ...
            ############################################################################
            layers['RAINlayer_lstm']    = self._LSTM2(tf.zeros((50,2048)), seq_length, feat_size=2048, cell_size=128, reuse=False)

            layers['RAINlayer_lstm_fc'] = self.FClayer(inputs=layers['RAINlayer_lstm'],
                                                     op_dims=1, reuse=False)

            layers['RAINlayer']         = self._extraction_layer(inputs=layers['124'], params=layers['RAINlayer_lstm_fc'], sets=j, K=k, L=seq_length)

            # Step 2
            layers['RAINlayer_lstm_2']    = self._LSTM2(layers['RAINlayer'], seq_length, feat_size=2048, cell_size=128, reuse=True)

            layers['RAINlayer_lstm_fc_2'] = self.FClayer(inputs=layers['RAINlayer_lstm_2'],
                                                     op_dims=1, reuse=True)
            layers['RAINlayer_2']         = self._extraction_layer(inputs=layers['124'], params=layers['RAINlayer_lstm_fc_2'], sets=j, K=k, L=seq_length)

            # Step 3
            layers['RAINlayer_lstm_3']    = self._LSTM2(layers['RAINlayer_2'], seq_length, feat_size=2048, cell_size=128, reuse=True)

            layers['RAINlayer_lstm_fc_3'] = self.FClayer(inputs=layers['RAINlayer_lstm_3'],
                                                     op_dims=1, reuse=True)
            layers['RAINlayer_3']         = self._extraction_layer(inputs=layers['124'], params=layers['RAINlayer_lstm_fc_3'], sets=j, K=k, L=seq_length)

            # Step 4
            layers['RAINlayer_lstm_4']    = self._LSTM2(layers['RAINlayer_3'], seq_length, feat_size=2048, cell_size=128, reuse=True)

            layers['RAINlayer_lstm_fc_4'] = self.FClayer(inputs=layers['RAINlayer_lstm_4'],
                                                     op_dims=1, reuse=True)
            layers['RAINlayer_4']         = self._extraction_layer(inputs=layers['124'], params=layers['RAINlayer_lstm_fc_4'], sets=j, K=k, L=seq_length)
            ############################################################################

            layers['125'] = self._LSTM(layers['RAINlayer_4'], seq_length, feat_size=2048, cell_size=256)
