" RESNET-50 + RAIN (INTERP + MEDIAN) v17 + LSTM MODEL IMPLEMENTATION FOR USE WITH TENSORFLOW "
...
class ResNet_RIL_Interp_Median_v17():
    ...
    def __init__(self, verbose=True):
        ...
        self.name = 'resnet_RIL_interp_median_model_v17'
        print "resnet RIL interp median v17 initialized"
    ...
    def inference(self, inputs, is_training, input_dims, output_dims, seq_length, scope, k, j, dropout_rate = 0.5, return_layer='logits', data_dict=None, weight_decay=0.0):
        ...
        if self.verbose:
            print('Generating RESNET RAIN INTERP MEDIAN v17 network layers')
	    ...
	    layers['RELU'] = tf.nn.relu(layers['Conv1_bn'])
            ...
            layers['POOL'] = max_pool_layer(tf.nn.relu(layers['Conv2']),
                                         filter_dims=[3, 3], stride_dims=[3,3],
                                         name='pool1', padding='VALID')

            layers['Reshape1'] = tf.reshape(layers['POOL'], (-1, k, 17, 17, 64))
            ...
            layers['Reshape3'] = tf.reshape(layers['FC1'], (-1, 17, 17, 64, 16))

            layers['Dimshuffle2'] = tf.transpose(layers['Reshape3'], (3,0,1,2,4))

            layers['Reshape4'] = tf.reshape(layers['Reshape3'], (64, -1))
...
if __name__ == '__main__':
    XX = ResNet_RIL_Interp_Median_v17()
