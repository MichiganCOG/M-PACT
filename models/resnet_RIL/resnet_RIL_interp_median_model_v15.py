" RESNET-50 + RAIN (INTERP + MEDIAN) v15 + LSTM MODEL IMPLEMENTATION FOR USE WITH TENSORFLOW "
...
class ResNet_RIL_Interp_Median_v15():
    ...
    def __init__(self, verbose=True):
        ...
        self.name = 'resnet_RIL_interp_median_model_v15'
        print "resnet RIL interp median v15 initialized"
    ...
    def _extraction_layer(self, inputs, params, sets, K, L):
        ...
        sample_alpha_tick = tf.nn.top_k(params[0][:,0], params[0][:,0].get_shape()[0]//2).values[params[0][:,0].get_shape()[0]//2 - 1]
        sample_phi_tick   = tf.nn.top_k(params[1][:,0], params[1][:,0].get_shape()[0]//2).values[params[1][:,0].get_shape()[0]//2 - 1]
    ...
    def inference(self, inputs, is_training, input_dims, output_dims, seq_length, scope, k, j, dropout_rate = 0.5, return_layer='logits', data_dict=None, weight_decay=0.0):
        ...
        if self.verbose:
            print('Generating RESNET RAIN INTERP MEDIAN v15 network layers')
        ...
        with tf.name_scope(scope, 'resnet', [inputs]):
            ...
            layers['FC2a'] = fully_connected_layer(input_tensor=layers['Reshape4'],
                    out_dim=1, non_linear_fn=tf.nn.sigmoid,
                    name='FC2a', weight_decay=weight_decay)

            layers['FC2b'] = fully_connected_layer(input_tensor=layers['Reshape4'],
                    out_dim=1, non_linear_fn=tf.nn.relu,
                    name='FC2b', weight_decay=weight_decay)

            layers['RIlayer'] = self._extraction_layer(inputs=inputs,
                                                       params=[layers['FC2a'], layers['FC2b']],
