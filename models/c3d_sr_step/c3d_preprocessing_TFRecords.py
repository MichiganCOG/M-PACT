...
def sinusoidal_resample(video, sample_dims, frame_count, tracker, num_vids, num_epochs, batch_size, num_clips, num_gpus, model_alpha):
    ...
    r_alpha = tf.cond(tf.greater(curr_epoch, num_epochs/2),
                    lambda: model_alpha  * tf.cast(frame_count, tf.float32) / float(sample_dims),
                    lambda: (alpha + (upper_limit - lower_limit) / 2.0 * tf.sin(tf.cast(tracker,tf.float32))) * tf.cast(frame_count, tf.float32) / float(sample_dims))
