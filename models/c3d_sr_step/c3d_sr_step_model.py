...
class C3D_SR_STEP():
    ...
    def inference(self, inputs, is_training, input_dims, output_dims, seq_length, scope, dropout_rate = 0.5, return_layer=['logits'], weight_decay=0.0):
        ...
        with tf.name_scope(scope, 'c3d', [inputs]):
            ...
            layers['Parameterization_Variables'] = tf.cond(tf.greater(curr_epoch, self.num_epochs/2),
                                lambda: self.model_alpha * tf.cast(self.clip_length, tf.float32) / float(self.input_dims),
                                lambda: (alpha + (upper_limit - lower_limit) / 2.0 * tf.sin(tf.cast(tracker,tf.float32))) * tf.cast(self.clip_length, tf.float32) / float(self.input_dims))
