...
def preprocess_for_train(image, output_height, output_width, resize_height, resize_width):
  """Preprocesses the given image for training.
  Args:
    image: A `Tensor` representing an image of arbitrary size.
    output_height: The height of the image after preprocessing.
    output_width: The width of the image after preprocessing.
    resize_height: The height of the image after the resize operation
    resize_width: The width of the image after the resize operation
  Returns:
...
def preprocess_for_eval(image, output_height, output_width, resize_height, resize_width):
...
def preprocess_image(image, output_height, output_width, resize_height, resize_width, is_training=False):
...
def preprocess(input_data_tensor, frames, height, width, channel, input_dims, output_dims, seq_length, size, label, istraining, input_alpha=1.0):
    ...
    input_data_tensor = tf.map_fn(lambda img: preprocess_image(img, size[0], size[1], 128, 171, is_training=istraining, resize_side_min=size[0]), input_data_tensor)
